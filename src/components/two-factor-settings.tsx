    ...
    <>
      <div className="flex flex-col gap-6">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Use an authenticator app to add a verification step when signing in.</p>
        </div>
        {enabled ? (
          <form onSubmit={disable}>
            <FieldGroup>
              <Field>
                <FieldLabel htmlFor="disable-2fa-password">Current password</FieldLabel>
                <Input id="disable-2fa-password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" required />
              </Field>
              <Button variant="destructive" type="submit" disabled={busy}>{busy ? 'Disabling...' : 'Disable two-factor authentication'}</Button>
            </FieldGroup>
          </form>
        ) : (
          <form onSubmit={beginSetup}>
            <FieldGroup>
              <Field>
                <FieldLabel htmlFor="enable-2fa-password">Current password</FieldLabel>
                <Input id="enable-2fa-password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" required />
                <FieldDescription>Scan a QR code with an authenticator app, then verify a six-digit code.</FieldDescription>
              </Field>
              <Button type="submit" disabled={busy}><KeyRound data-icon="inline-start" />{busy ? 'Preparing...' : 'Set up two-factor authentication'}</Button>
            </FieldGroup>
          </form>
        )}
      </div>
      <Dialog open={Boolean(setup)} onOpenChange={(open) => { if (!open) { setSetup(null); setCodeReady(false) } }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Set up authenticator app</DialogTitle>
            <DialogDescription>Scan this QR code, save your recovery codes somewhere secure, then enter the code shown by your authenticator app.</DialogDescription>
          </DialogHeader>
          {setup && (
            <form onSubmit={confirmSetup} className="grid gap-5">
              <div className="grid justify-items-center gap-3">
                <img src={setup.image} alt="Authenticator app QR code" className="size-50 rounded-lg border bg-white p-2" />
                <code className="max-w-full break-all rounded bg-muted px-2 py-1 text-xs">{setup.totpURI}</code>
              </div>
              <div className="rounded-lg border bg-muted/30 p-3">
                <p className="mb-2 text-sm font-medium">Recovery codes</p>
                <div className="grid grid-cols-2 gap-1 font-mono text-xs">{setup.backupCodes.map((backupCode) => <span key={backupCode}>{backupCode}</span>)}</div>
              </div>
              <Field>
                <FieldLabel htmlFor="two-factor-code">Verification code</FieldLabel>
                <TwoFactorCodeInput key={setup.totpURI} disabled={busy} onChange={(value) => setCodeReady(value.length === 6)} />
              </Field>
              <DialogFooter>
                <Button type="submit" disabled={busy || !codeReady}>{busy ? 'Verifying...' : 'Verify and enable'}</Button>
              </DialogFooter>
            </form>
          )}
        </DialogContent>
