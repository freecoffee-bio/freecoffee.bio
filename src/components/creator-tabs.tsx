...
export type Creator = { name: string; coverImageUrl?: string | null; currency?: string; bio?: string | null; whatDo?: string | null; image?: string | null; website?: string | null; socialLinks?: string | null; welcomeMessage?: string | null; defaultSupportAmount?: number; suggestedSupportAmounts?: string | null; minimumSupportAmount?: number; supportWording?: 'tip' | 'donate'; terms?: string | null; allowAnonymous?: boolean; showSupport?: boolean; showShop?: boolean; supportGoal?: SupportGoal | null; products?: Product[]; gallery?: GalleryItem[]; posts?: Post[]; supporters?: Supporter[]; paymentProviders?: { stripe: boolean; paypal: boolean; baseUsdc: boolean; solanaUsdc: boolean; solanaUsdt: boolean } }
...
export function PostsTab({ creator, isAdmin, currentUser }: Pick<TabProps, 'creator' | 'isAdmin' | 'currentUser'>) {
  ...
  return <>
    ...
    {creator.posts?.length ? <section className="grid justify-items-start gap-4 sm:grid-cols-2 lg:grid-cols-3">{creator.posts.map((post) => { const href = `/posts/${encodeURIComponent(post.id)}`; return <article className="relative flex aspect-[300/379] w-full max-w-[300px] flex-col overflow-hidden rounded-xl border bg-card shadow-sm" key={post.id}><a href={href} className="flex min-h-0 flex-1 flex-col" aria-label={post.title}><div className="relative"><CardCover src={post.coverImageUrl} alt={`${post.title} cover`}><FileText className="size-10 text-muted-foreground/60" /></CardCover>{isAdmin && post.status === 'draft' && <Badge className="absolute bottom-2 right-2 border-primary bg-primary text-primary-foreground">Draft</Badge>}</div><div className="min-h-0 flex-1 overflow-hidden p-4"><p className="font-mono text-xs text-muted-foreground">{post.publishedAt ? new Date(post.publishedAt).toLocaleDateString() : ''}</p><h2 className="mt-2 truncate text-lg font-semibold">{post.title}</h2><p className="mt-2 line-clamp-2 text-sm leading-5 text-muted-foreground">{post.excerpt || post.body}</p></div></a>{isAdmin && <CardActions onEdit={() => setEditing(post)} onDelete={() => setDeleting(post.id)} />}</article> })}</section> : <Placeholder title="Posts" text="This creator has not published any posts yet." />}
...
export function ShopTab({ creator, currentUser, isAdmin }: Pick<TabProps, 'creator' | 'currentUser' | 'isAdmin'>) {
  const hasPaymentProvider = Boolean(
    creator.paymentProviders?.stripe
    || creator.paymentProviders?.paypal
    || creator.paymentProviders?.baseUsdc
    || creator.paymentProviders?.solanaUsdc
    || creator.paymentProviders?.solanaUsdt,
  )
  ...
  return <>
    ...
    {creator.showShop === false ? <Placeholder title="Shop" text="This creator has hidden their shop." /> : creator.products?.length ? <section className="grid items-start justify-items-start gap-4 sm:grid-cols-2 lg:grid-cols-3">{creator.products.map((product) => <article className="relative flex aspect-[300/379] w-full max-w-[300px] min-w-0 flex-col overflow-hidden rounded-2xl border bg-card shadow-sm" key={product.id}><div className="relative"><a href={`/shop/${encodeURIComponent(product.id)}`} className="block"><CardCover src={product.coverImageUrl} alt={`${product.name} cover`}><ShoppingBag className="size-10 text-muted-foreground/60" /></CardCover></a><div className="pointer-events-none absolute inset-x-2 bottom-2 flex items-center justify-between gap-2"><span className={`max-w-[62%] truncate rounded-sm border border-white/70 px-2 py-1 text-[11px] font-bold uppercase text-white shadow-sm backdrop-blur-sm ${hasPaymentProvider ? 'bg-black/55' : 'bg-black/65'}`}>{hasPaymentProvider ? 'Digital' : 'Unavailable'}</span><span className="shrink-0 rounded-sm border border-white/70 bg-primary px-2 py-1 text-xs font-semibold text-primary-foreground shadow-sm">{product.price === 0 ? 'Free +' : `${displayAmount(product.price, product.currency)} ${product.currency}`}</span></div></div>{isAdmin && <CardActions onEdit={() => setEditing(product)} onDelete={() => setDeleting(product.id)} />}<div className="min-h-0 bg-card p-3"><div className="min-w-0"><h2 className="truncate text-lg font-semibold leading-6"><a href={`/shop/${encodeURIComponent(product.id)}`} className="hover:underline">{product.name}</a></h2><p className="mt-1 truncate text-sm leading-5 text-muted-foreground">{product.description || 'Digital download'}</p></div></div></article>)}</section> : <Placeholder title="Shop" text="No products yet. Check back soon for digital tools and resources." />}
