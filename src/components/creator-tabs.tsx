...
export function ShopTab({ creator, currentUser, isAdmin }: Pick<TabProps, 'creator' | 'currentUser' | 'isAdmin'>) {
  ...
  return <>
    ...
    {creator.showShop === false ? <Placeholder title="Shop" text="This creator has hidden their shop." /> : creator.products?.length ? <section className="grid items-start justify-items-start gap-4 sm:grid-cols-2 lg:grid-cols-3">{creator.products.map((product) => <article className="relative flex aspect-[300/379] w-full max-w-[300px] min-w-0 flex-col overflow-hidden rounded-2xl border bg-card shadow-sm" key={product.id}><div className="relative"><a href={`/shop/${encodeURIComponent(product.id)}`} className="block"><CardCover src={product.coverImageUrl} alt={`${product.name} cover`}><ShoppingBag className="size-10 text-muted-foreground/60" /></CardCover></a><div className="pointer-events-none absolute inset-x-2 bottom-2 flex items-center justify-between gap-2"><span className={`max-w-[62%] truncate rounded-sm border border-white/70 px-2 py-1 text-[11px] font-bold uppercase text-white shadow-sm backdrop-blur-sm ${creator.paymentProviders?.stripe || creator.paymentProviders?.paypal || creator.paymentProviders?.baseUsdc ? 'bg-black/55' : 'bg-black/65'}`}>{creator.paymentProviders?.stripe || creator.paymentProviders?.paypal || creator.paymentProviders?.baseUsdc ? 'Digital' : 'Unavailable'}</span><span className="shrink-0 rounded-sm border border-white/70 bg-primary px-2 py-1 text-xs font-semibold text-primary-foreground shadow-sm">{product.price === 0 ? 'Free +' : `${displayAmount(product.price, product.currency)} ${product.currency}`}</span></div></div>{isAdmin && <CardActions onEdit={() => setEditing(product)} onDelete={() => setDeleting(product.id)} />}<div className="min-h-0 bg-card p-3"><div className="min-w-0"><h2 className="truncate text-lg font-semibold leading-6"><a href={`/shop/${encodeURIComponent(product.id)}`} className="hover:underline">{product.name}</a></h2><p className="mt-1 truncate text-sm leading-5 text-muted-foreground">{product.description || 'Digital download'}</p></div></div></article>)}</section> : <Placeholder title="Shop" text="No products yet. Check back soon for digital tools and resources." />}
