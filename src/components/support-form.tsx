...
type SupportFormProps = {
  creator: { name: string; allowAnonymous?: boolean; paymentProviders?: { stripe: boolean; paypal: boolean; baseUsdc: boolean; solanaUsdc: boolean; solanaUsdt: boolean } }
...
const schema = z.object({
  ...
  anonymous: z.boolean(),
  provider: z.enum(['stripe', 'paypal', 'base-usdc', 'solana-usdc', 'solana-usdt']),
})
...
export function SupportForm({ creator, currentUser, defaultSupportAmount = 500, suggestedSupportAmounts, minimumSupportAmount = 100, supportWording = 'donate', currency = 'USD', onSubmitted }: SupportFormProps) {
  ...
  const providers = creator.paymentProviders ?? { stripe: false, paypal: false, baseUsdc: false, solanaUsdc: false, solanaUsdt: false }
  const availableProviders = (['stripe', 'paypal', 'base-usdc', 'solana-usdc', 'solana-usdt'] as const).filter((provider) => provider === 'base-usdc' ? providers.baseUsdc : provider === 'solana-usdc' ? providers.solanaUsdc : provider === 'solana-usdt' ? providers.solanaUsdt : providers[provider])
