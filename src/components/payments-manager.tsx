...
function PaymentList({ items, total, page, onPageChange, type, onTypeChange }: { items: PaymentItem[]; total: number; page: number; onPageChange: (page: number) => void; type: string; onTypeChange: (type: string) => void }) {
  ...
  const showDetails = async (item: PaymentItem) => {
    setSelected(item)
    if (details[item.id] || loadingId === item.id) return

    setErrors((current) => { const { [item.id]: _, ...rest } = current; return rest })
    setLoadingId(item.id)
    const controller = new AbortController()
    const timeout = window.setTimeout(() => controller.abort(), 15_000)
    try {
      const response = await fetch(`/api/admin/transactions?${item.kind === 'order' ? 'orderId' : 'supportId'}=${encodeURIComponent(item.id)}`, { signal: controller.signal })
      const body = await response.json().catch(() => ({})) as Partial<PaymentDetails> & { error?: string; requestId?: string }
      if (!response.ok) throw new Error(body.error || `Unable to load payment details (HTTP ${response.status}).`)
      setDetails((current) => ({ ...current, [item.id]: { items: body.items ?? [], payments: body.payments ?? [], events: body.events ?? [] } }))
    } catch (error) {
